...
import de.tum.in.ase.eist.car.DragonCar;
...
import de.tum.in.ase.eist.collision.NewCollision;
...
public class GameBoard {
	...
	private static final int NUMBER_OF_DRAGON_CARS=1;
	...
	private void createCars() {
		...
		for (int i = 0; i <NUMBER_OF_DRAGON_CARS ; i++) {
			this.cars.add(new DragonCar(this.size));
	...
	public void moveCars() {
		...
		for (Car car : cars) {
			...
			Collision specialCollision = new NewCollision(player.getCar(), car);
			if (specialCollision.isCrash()) {
				Car winner = specialCollision.evaluate();
				Car loser = specialCollision.evaluateLoser();
